...
CodeVetter SHALL render Home, Review, Repo, Agents, T-Rex, and Settings through one semantic dark surface system with shared background, panel, border, text, status, focus, radius, and spacing tokens. Amber SHALL be the only routine selection and primary-action accent; semantic warning, danger, and success colors MAY communicate state but MUST NOT decorate navigation categories.

#### Scenario: User moves between primary surfaces
- **WHEN** the user navigates across all six primary destinations
...
The redesign MUST preserve existing routes, URL parameters, Tauri/browser guards, data loading, commands, review and fix flows, graph/history interactions, terminal creation and control, scenario compilation, settings persistence, command palette, and updater behavior. It MAY reorder or disclose existing panels when native review shows that the primary action is buried, provided no data or evidence state is removed.

#### Scenario: Existing browser regression suite runs
- **WHEN** the focused route, navigation, Review, Repo, Agents, T-Rex, and Settings browser tests run against the redesigned shell
